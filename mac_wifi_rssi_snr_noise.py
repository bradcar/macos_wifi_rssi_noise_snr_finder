# mac_wifi_rssi_snr_noise.py
"""
On macOS, repeatedly measure and print RSSI, Noise, and SNR
of the currently connected network on interface en0.

usage:
  python3 mac_wifi_rssi_snr_noise.py
...
def query_wifi():
    """
    MacOS query rssi and noise on WiFi.
...
def rssi_snr_noise_to_string(rssi, snr, noise) -> tuple[str, str, str]:
    # RSSI text, higher is better
    if rssi > -50:
        rssi_string = "4 bars"
    elif rssi > -60:
        rssi_string = "3 bars"
    ...
    # Noise level, lower (more neg) is better
    if noise <= -90:
    ...
    if snr > 30:
        ...
        snr_string = "poor"
    return rssi_string, snr_string, noise_string
...
def print_with_string(noise, rssi, ssid):
    """
    Prints RSSI, noise and SNR with text interpretation
    While SNR is ratio, because of log scales this is a subtraction.
    ...
    print(f"WiFi Signal Monitor (macOS): {ssid}")
    print(f"{ssid=}")
